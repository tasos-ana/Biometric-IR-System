...
public class TextWriter {
    ...
    private File prepareFiles(final String rootPath, final String targetFile) {
        ...
        final File f = new File(rootPath + "\\" + targetFile);
    ...
    public String store() throws IOException {
        ...
        try {
            final RandomAccessFile file = new RandomAccessFile(postingFile, "rw");

            // calculate norms while writing posting file and vocabulary
            data.getWords().store(vocabularyWriter, file, data.getNorms(), data.getDocuments().size());
        ...
        return "Vocabulary and Posting File created in: \t" + elapsedSeconds + " seconds";
    }
